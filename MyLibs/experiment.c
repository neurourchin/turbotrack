 ...
 * but WITHOUT ANY WARRANTY; without even the impliedƒ warranty of
...
/*
 * Release the memory associated with window names
...
void SetupGUI(Experiment* exp) {
	...
	if (exp->FluorMode){
		cvResizeWindow(exp->WinCon1, 600, 450);
	} else {
...
/* 	if (exp->FluorMode){
		cvResizeWindow(exp->WinCon2, 600, 450);
	}else{
...
int GrabFrame(Experiment* exp) {
	...
	if (!(exp->VidFromFile)) {
		...
		if (exp->UseFrameGrabber) {
			...
			//intf("Trying to acquire a frame...\n");
...
void DoSegmentation(Experiment* exp) {
	...
	if (!(exp->e)) 
		FindWormBoundary(exp->Worm,exp->Params, exp->Worm->FluorFeatures->centroid); // ,exp->PrevWorm modified by Ni: use prevworm information to crop region of interest out of full image
	TICTOC::timer().toc("_FindWormBoundary",exp->e);

	/** Store worm pos from prev 6 frames**/
	if (AddWormMotionHistory(exp->Worm->TimeEvolution,exp->Worm->currvelocity,exp->Params)!=A_OK) printf("Error adding mean curvature!!\n");
	

	/** Display values in buffer **/
	double* wormVelBuff=NULL;
	SeqDoublesToArr((const CvSeq*) exp->Worm->TimeEvolution->WormVelBuffer,&wormVelBuff);

		//printf("Current worm vel: (%d,%d)\n",(exp->Worm->currvelocity.x)*10000,(exp->Worm->currvelocity.y)*10000);

	/** Calculate most recent acceleration **/
	//CvPoint tempacc;
	//tempacc = cvPoint((wormVelBuff[1].x-wormVelBuff[2].x),wormVelBuff[1].y-wormVelBuff[2].y))
	//exp->Worm->TimeEvolution->RecentAcceleration=tempacc/((clock()- exp->prevTime)/CLOCKS_PER_SEC);

//printf("Current worm acceleration: (%f,%f)\n",exp->Worm->TimeEvolution->RecentAccelerationy);
	
	/** Deallocate memory for worm motion buffers **/
	free(wormVelBuff);
...
void MarkRecenteringTarget(Experiment* exp){
	// int A = (int)(exp->stageFeedbackTarget.x);
	// int B = (int)(exp->stageFeedbackTarget.y);
...
//printf("stageFeedbackTarget: x=%d, y=%d, A=%d, B=%d\n",exp->stageFeedbackTarget.x,exp->stageFeedbackTarget.y, A, B);
	...
	//cvDrawContours(exp->Worm->ImgThresh,exp->Worm->Boundary,cvScalar(255,255,255),cvScalar(255,255,255),5,1,CV_AA);
 ...
 */
 /* 7/12/2016: Ni is changing the algorithm for determining stage velocity: the worm's previous velocity and acceleration will be stored
 * and passed on AdjustStageToKeepObjectAtTarget to computer the next velocity command to send to the stage
 */
