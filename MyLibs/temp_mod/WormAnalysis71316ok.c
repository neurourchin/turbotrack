...
WormTimeEvolution* CreateWormTimeEvolution(){
	...
	TimeEv->WormPosBuffer=cvCreateSeq(0,sizeof(CvSeq),sizeof(double),TimeEv->MemTimeEvolutionStorage);
	TimeEv->derivativeOfHeadCurvature=0;
	TimeEv->currMeanHeadCurvature=0;
 ...
 *
 */
void FindWormBoundary(WormAnalysisData* Worm, WormAnalysisParam* Params, CvPoint* prevpt){ // prevpt is the previous centroid of the fluorescent feature that remains in Worm->FF->centroid
	...
	//if (Params->ApertureOn) {
		...
		cvZero(CircleROI);
		 if (((*prevpt).y) !=0){
			 ...
			 //printf("Previous centroid is (%d,%d) \n",x,y);//,PrevWorm->centroid->x,PrevWorm->centroid->y);
		 } else {
...
//	cvErode(Worm->ImgSmooth, Worm->ImgSmooth,NULL,2);
	...
	if (Params->FluorMode){ 
		...
		*(Worm->currvelocity) = cvPoint((Worm->FluorFeatures->centroid->x - prevpt->x),(Worm->FluorFeatures->centroid->y - prevpt->y));
