...
WormAnalysisParam* CreateWormAnalysisParam(){
	...
	ParamPtr->ApertureX=545/2; // x coordinate of center of circle   //1024/2
	ParamPtr->ApertureY=429/2; // y coordinate of center of circle   //768/2
	ParamPtr->ApertureR=429/2; // radius of circle   //768/2
 ...
 *
 */
void FindWormBoundary(WormAnalysisData* Worm, WormAnalysisParam* Params,WormGeom* PrevWorm){
	...
	CvPoint Ct = cvPoint(250,200);
		/** draw a filled in circle for a mask **/
		IplImage* CircleROI=cvCreateImage(cvGetSize(Worm->ImgOrig),IPL_DEPTH_8U,1);
		cvZero(CircleROI);
		if ((PrevWorm->centroid->y) !=0){
			//PrevCt = *(PrevWorm->FluorFeatures->centroid);
			cvCircle(CircleROI,*(PrevWorm->centroid),100,cvScalar(255,255,255),-1,CV_AA,0);
		printf("Previous centroid is (%d,%d) \n",PrevWorm->centroid->x,PrevWorm->centroid->y);
		} else {
...
//	cvErode(Worm->ImgSmooth, Worm->ImgSmooth,NULL,2);
	...
	//CvSize dim2 = cvGetSize(Worm->ImgThresh);
		//printf("imgthresh size is: %d,%d \n", dim2.width, dim2.height);
	...
	/** Find Contours **/
	CvSeq* contours;
	IplImage* TempImage=cvCreateImage(cvGetSize(Worm->ImgThresh),IPL_DEPTH_8U,1); // TempImage Created
	...
	/** Ni: Find contour with largest area **/
	TICTOC::timer().tic("cvlarestContour");
	if (contours) largestContour(contours,&rough);
	TICTOC::timer().toc("cvLargestContour");
	cvReleaseImage(&TempImage);
printf("found largest boundary \n");
	/** Smooth the Boundary **/
	// if (Params->BoundSmoothSize>0){
		...
		// Worm->Boundary=cvCloneSeq(smooth); // this is where worm boundary record is generated --Ni
	...
	if (Params->FluorMode){ 
		...
		if (Worm->FluorFeatures->centroid != NULL) {
			...
			*(Worm->FluorFeatures->centroid)=cvPoint(Worm->FluorFeatures->moments->m10/Worm->FluorFeatures->moments->m00,Worm->FluorFeatures->moments->m01/Worm->FluorFeatures->moments->m00);
			...
			printf("done finding current centroid \n");
	}
...
int CreateWormHUDS(IplImage* TempImage, WormAnalysisData* Worm, WormAnalysisParam* Params, Frame* IlluminationFrame){
	...
	if (!(Params->FluorMode)){
		...
		if (Worm->FluorFeatures!=NULL && Worm->isPresent==1) {
				cvCircle(TempImage,*(Worm->FluorFeatures->centroid),CircleDiameterSize*2,cvScalar(255,0,0),2,CV_AA,0);
				cvDrawContours(TempImage,Worm->Boundary,cvScalar(255,255,255),cvScalar(255,255,255),5,1,CV_AA);
		printf("Centroid found! (%d,%d)\n", Worm->FluorFeatures->centroid->x,Worm->FluorFeatures->centroid->y);
		//CvSize dim = cvGetSize(TempImage);
		//printf("TempImage size is: %d,%d \n", dim.width, dim.height);
		} 	
		else if (Worm->FluorFeatures==NULL && Worm->isPresent==1){
			printf("No centroid found to draw!\n");
	}
		else {
			printf("Worm struct not present!\n");
		}
	}
...
void DisplayWormSegmentation(WormAnalysisData* Worm, IplImage* ImgOut){
	...
	// for (i = 0; i < Worm->Segmented->Centerline->total; i++) {
		...
		//cvDrawContours(TempImage,Worm->Boundary,cvScalar(0,0,0),cvScalar(0,0,0),1,1,CV_AA);
...
void ClearWormGeom(WormGeom* SimpleWorm){
	...
	SimpleWorm->Tail.x=0;
	SimpleWorm->Tail.y=0;
	SimpleWorm->centroid->x = 0;
	SimpleWorm->centroid->y = 0;
}
...
void LoadWormGeom(WormGeom* SimpleWorm, WormAnalysisData* Worm){
	...
	//printf("LoadingCurrentWorm2Prev\n");
	 ClearWormGeom(SimpleWorm);
