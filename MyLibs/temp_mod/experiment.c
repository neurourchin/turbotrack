...
void DoSegmentation(Experiment* exp) {
	...
	if (!(exp->e)) 
		FindWormBoundary(exp->Worm,exp->Params,exp->PrevWorm); // modified by Ni: use prevworm information to crop region of interest out of full image
	...
	if (!(exp->e))
	LoadWormGeom(exp->PrevWorm, exp->Worm);  // i.e. assign current worm struct to previous worm struct
